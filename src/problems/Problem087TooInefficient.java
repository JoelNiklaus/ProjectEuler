...
public class Problem087TooInefficient {
	...
	private static boolean isPrimePowerTripleWithArray(int number) {
		int length = primesArray.length;
		for (int x = 0; x < length; x++)
			for (int y = 0; y < length; y++)
				for (int z = 0; z < length; z++)
					if ((int) (Math.pow(primesArray[x], 2) + Math.pow(primesArray[y], 3) + Math
							.pow(primesArray[z], 4)) == number)
						// System.out.println(number + " = " + primes.get(x) + "^2 + " +
						// primes.get(y) + "^3 + " + primes.get(z) + "^4");
						return true;
